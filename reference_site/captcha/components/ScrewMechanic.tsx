...
const PixelNut: React.FC<{ x: number; angle: number; isGhost?: boolean }> = ({ x, angle, isGhost }) => {
    ...
    const strokeColor = isGhost ? '#00e756' : 'black'; // Green outline for ghost target
    const fillColor = isGhost ? 'transparent' : '#b13e53'; // Rust red for the nut
    const detailColor = isGhost ? '#00e756' : '#ffd700'; // Gold/Green details
...
export const ScrewMechanic: React.FC<ScrewMechanicProps> = ({ rotation, targetRotation = 0, showTarget = true }) => {
  ...
  return (
    <div className="relative w-full h-48 flex items-center justify-center overflow-hidden bg-retro-bg rounded border-2 border-black shadow-pixel-inset">
        {/* SVG Container */}
